...
static int do_sync(clockid_t clkid, int cmdc, char *cmdv[])
{
	struct timespec ts;
	memset(&ts, 0, sizeof(ts));
	if (clock_gettime(clkid, &ts))
	{
		pr_err("get: failed to get phc clock time: %s",
			   strerror(errno));
	...
	{
		pr_notice("phc clock time is %lld.%09lu or %s",
				  (long long)ts.tv_sec, ts.tv_nsec, ctime(&ts.tv_sec));
	}
	...
	{
		pr_notice("set CLOCK_REALTIME clock time to %lld.%09ld or %s",
				  (long long)ts.tv_sec, ts.tv_nsec, ctime(&ts.tv_sec));
	}
	return 0;
}
...
struct SysoffResult *SysoffEstimateBasic(struct timespec *ts1, struct timespec *rt, struct timespec *ts2)
{
	struct SysoffResult *res;
	...
	// 计算系统时间
	struct timespec sysTime;
	sysTime.tv_sec = ts1->tv_sec + interval.tv_sec / 2;
	sysTime.tv_nsec = ts1->tv_nsec + interval.tv_nsec / 2;

	// 计算偏移量
	struct timespec offset;
	offset.tv_sec = sysTime.tv_sec - rt->tv_sec;
	offset.tv_nsec = abs(sysTime.tv_nsec - rt->tv_nsec);
	if (sysTime.tv_nsec > rt->tv_nsec)
	{
		res->flag = 1;
	}
	else
...
{
	struct timespec ts1, ts, ts2;

	memset(&ts, 0, sizeof(ts));
	memset(&ts1, 0, sizeof(ts1));
	memset(&ts2, 0, sizeof(ts2));

	if (clock_gettime(CLOCK_REALTIME, &ts1) || clock_gettime(clkid, &ts) || clock_gettime(CLOCK_REALTIME, &ts2))
	{
	...
	{
		struct SysoffResult *sr = SysoffEstimateBasic(&ts1, &ts, &ts2);

		pr_notice("SYS clock time is %lld.%09lu or %s",
				  (long long)sr->SysTime.tv_sec, sr->SysTime.tv_nsec, ctime(&(sr->SysTime.tv_sec)));

		pr_notice("PHC clock time is %lld.%09lu or %s",
				  (long long)sr->PHCTime.tv_sec, sr->PHCTime.tv_nsec, ctime(&(sr->PHCTime.tv_sec)));

		pr_notice("Delay is %lld.%09lu\n", (long long)sr->Delay.tv_sec, sr->Delay.tv_nsec);
		pr_notice("Offset is %c%lld.%09lu\n", sr->flag ? '+' : '-', (long long)sr->Offset.tv_sec, sr->Offset.tv_nsec);
